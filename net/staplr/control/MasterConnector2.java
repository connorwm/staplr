...
public class MasterConnector2 
{
	private Settings s_settings;
	private Log l_log;
	private TextFieldLogger tf_logger;
	private Communicator l_listener;
	...
	private void initialize() {
		...
		// Create a blank log file to be a placeholder
		// Logging in this window is handler by the 
		// textfield logger
		l_log = new Log("control.log");
		l_log.setOption(Options.ConsoleOutput, false);
		l_log.setOption(Options.FileOutput, false);
		
		s_settings = new Settings(l_log);
		...
		if(s_settings.loaded())
		{
			...
			l_listener = new Communicator(s_settings, 2000, Type.Service, null, null);
			t_listener = new Thread(l_listener);
							...
							for(int i_masterIndex = 0; i_masterIndex < s_settings.c_credentials.size(); i_masterIndex++)
							{
								...
								if(((String)c_credential.get(Properties.location)).equals(str_location))
								{
									if(String.valueOf(c_credential.get(Properties.port)).equals(str_port))
									{
										...
										if((mx_executor = (MessageExecutor)l_listener.connect(c_credential)) != null)
										{
